function  [dID,k_Frames_test,k_Frames_retest,dIdent] = f_compute_Idiff_frames(dFCw_2D_Test,dFCw_2D_Retest,configs,fi)
...
if fi == 1
    figure;imagesc(dIdent);title('dynamic identifiability matrix');axis image
end
aux = dIdent;
...
for k=1:length(k_frames)
    for s1=1:configs.numSubj
        for s2=1:configs.numSubj
            ...
            Ident_mean(s1,s2) = mean(tmp_fltd(:),'omitnan');
        end
