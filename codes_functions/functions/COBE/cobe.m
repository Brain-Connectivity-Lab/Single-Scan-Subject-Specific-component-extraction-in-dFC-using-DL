function [Ac, Bc, f, Zc] = cobe( Y,opts )
% Common orthogonal basis extraction
defopts=struct('c',[],'maxiter',200,'PCAdim',[],'tol',1e-6,'epsilon',0.01);%% Change the epsilon value to .03- Suggested by Rajan for rsfMRI
if ~exist('opts','var')
    opts=struct;
end
...
%     Ac(:,j)=U{1}*randn(size(U{1},2),1);
    ...
    %% stopping criterion
    if isempty(c)  %% terminate automatically
        if res(j)>epsilon
            res(j)=inf;
            break;
        end
    else
        if j>c    %% terminate if c common components have been found
            break;
        end
    end
